...
def get_ImageNetdataloaders(train_data_path, test_data_path):
    ...
    client_train_loaders = DataLoader(CustomImageNet(train_image_data, train_label_data, transform), batch_size=local_bs, shuffle=True,num_workers=4)
    ...
    client_test_loader = DataLoader(CustomImageNet(test_image_data, test_label_data, transform), batch_size=bs, shuffle=True,num_workers=4)
...
def get_serverdataloader(server_test_path):
    ...
    for i in range(len(lablename_list)):
        ...
        for j in range(len(file_names)):
            test_label_data.append(cla_dict[lablename_list[i]])
    server_test_loader = DataLoader(CustomImageNet(test_image_data, test_label_data, transform), batch_size=bs, shuffle=True,num_workers=4)
