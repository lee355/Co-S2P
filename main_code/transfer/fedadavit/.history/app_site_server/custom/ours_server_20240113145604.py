...
class ScatterAndGather_Ours(Controller):
    def __init__(
        ...
        model_dict = torch.load("/home/xugw/pretrained_model/AdaVIT_pretrained_model_with_patch2embedandqkv.pth")
        self.server = Server(self.test_loader, drop, num_classes , embed_dim , transformer_depth , transformer_head, mlp_dim ,img_size, lambda1, temperature, model_rate=1, device=self.device,model_dict=model_dict)
        self.param_idx = None
        self.loss_result = []
        self.top_1_result = []
        self.top_5_result = []
