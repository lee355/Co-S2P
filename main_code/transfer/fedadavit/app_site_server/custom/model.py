...
def get_server_model(drop, img_size, num_classes, embed_dim, transformer_depth, transformer_head, mlp_dim,device, segment_mask, self_distillation, model_rate=1):
    ...
    model = AdaStepT2T_ViT(img_size=img_size, patch_size=16, in_chans=3, num_classes=num_classes, 
                                   embed_dim=embed_dim, depth=math.floor(transformer_depth*model_rate), num_heads=transformer_head, mlp_ratio=mlp_ratio).to(device)
