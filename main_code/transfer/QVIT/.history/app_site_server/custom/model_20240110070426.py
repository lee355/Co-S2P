...
def get_model(args, img_size):
    ...
    model = QuantVisionTransformer(img_size=img_size[1], patch_size=8, in_chans=img_size[0], num_classes=args.num_classes, 
                                   embed_dim=args.embed_dim, depth=args.transformer_depth, num_heads=args.transformer_head, mlp_ratio=args.mlp_ratio, 
                                   norm_layer=partial(nn.LayerNorm, eps=1e-6), wbits=4, abits=4).to(args.device)
