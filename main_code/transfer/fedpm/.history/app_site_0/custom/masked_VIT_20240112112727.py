    ...
    def forward(self, x, ths=None):
        if self.is_bias == True:
            if ths is None:
                ...
                g_m = torch.where(nd_w_mask > ths, 1, 0)   #中位�?
...
class exist_classifier(nn.Module):
    def __init__(self, dim, mlp_dim, num_classes, training_mode):
        super().__init__()
        self.training_mode = training_mode
        self.exist_linear1 = nn.Linear(dim, mlp_dim)
        self.drop = nn.Dropout(0.5)
        self.gelu = nn.GELU()
        self.exist_linear2 = nn.Linear(mlp_dim, num_classes)
    def forward(self, x, ths=None):
        x = self.exist_linear1(x)
        x = self.drop(x)
        x = self.gelu(x)
        x = self.exist_linear2(x)
        return x


class ViT(nn.Module):
    def __init__(self, *, training_mode, image_size, patch_size, num_classes, dim, depth, full_depth, heads, mlp_dim, self_distillation=True, channels=3):
        super().__init__()
        ...
        self.patch_to_embedding = PatchEmbed(embed_dim=dim) #TODOmaskedlinear
