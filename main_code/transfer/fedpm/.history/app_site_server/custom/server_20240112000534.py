    ...
    def sample_mask_aggregation(self, sampled_mask, local_freq, num_params, n_roud):
        ...
        with torch.no_grad():
            ...
            for client in sampled_mask.keys():
                sampled_mask_client, local_freq_client, num_params_client = sampled_mask[client], local_freq[client], num_params[client]  #TODO
                avg_freq += local_freq_client
                local_bitrate = self.find_bitrate([local_freq_client + 1e-50, 1 - local_freq_client + 1e-50], num_params_client) + math.log2(num_params_client)
                avg_bitrate += local_bitrate / num_params_client
                for k, v in sampled_mask_client.items():
                    if 'mask' in k:
                        self.alphas[k] += v
