...
class Client:
    ...
    def __init__(self, drop, img_size, num_classes, embed_dim, mlp_dim, transformer_depth, transformer_head, 
        ...
        self.local_weight_model = FedModel(drop=self.drop, img_size=self.img_size, num_classes=self.num_classes,
        ...
        self.model_size = self.local_weight_model.model_size  # bit
    ...
    def get_local_weights(self, weight_or_mask):
        if weight_or_mask == "weight":
            return self.local_weight_model.get_weights()
        elif weight_or_mask == "mask":
            return self.local_mask_model.get_weights()
    ...
    def train_local(self, n_round, initial_weights, local_ep):
        ...
        if n_round <= 1:
            ...
            self.local_mask_model = FedModel(drop=self.drop, img_size=self.img_size, num_classes=self.num_classes,
                                    embed_dim=self.embed_dim, transformer_depth=self.transformer_depth, transformer_head=self.transformer_head, mlp_dim=self.mlp_dim, 
                                    lambda1=self.lambda1, temperature=self.temperature,
                                    training_mode=self.training_mode, self_distillation=False, 
                                    weight_decay=self.weight_decay, no_prox=self.no_prox, lr=self.lr, 
                                    model_rate=self.ratio, device=self.device)
            self.local_mask_model.model.load_state_dict(copy.deepcopy(initial_weights), strict=False)
            if n_round != 0:
                self.load_local_mask_and_classifier(weight_or_mask="mask")
 
            loss_mask = self.local_mask_model.train_mask(data_loader=self.local_train_dataloader, local_sub_epoch=20)
            self.save_local_mask_and_classifier(weight_or_mask="mask")
            
            mask_model_weights = self.get_local_weights(weight_or_mask="mask")
            self.mask_indices = {}
            ...
            self.local_weight_model = FedModel(drop=self.drop, img_size=self.img_size, num_classes=self.num_classes, segment_mask=self.single_mask_indices,
            ...
            self.local_weight_model.model.load_state_dict(mask_model_weights, strict=False)
            loss_weight = self.local_weight_model.train_weights(data_loader=self.local_train_dataloader, 
                                                        local_sub_epoch=local_ep-mask_rounds,training_round=self.training_round)
        else:
            print(type(self.local_weight_model.model))
            self.load_local_mask_and_classifier(weight_or_mask="weight")  
            ...
            for k, v in self.mask_indices.items():
                ...
                if "weight" in k:
                    ...
                    weight_model_weights[k] = weight_model_weights[k][v.bool()]
            #start train weight model
            self.local_weight_model.model.load_state_dict(weight_model_weights, strict=False)
            loss_weight = self.local_weight_model.train_weights(data_loader=self.local_train_dataloader, 
                                                        local_sub_epoch=self.args.local_ep, training_round=self.training_round)


    def save_local_mask_and_classifier(self, weight_or_mask):
        local_model_state_dict = self.get_local_weights(weight_or_mask=weight_or_mask)
    ...
    def load_local_mask_and_classifier(self, weight_or_mask):
        if weight_or_mask == "mask":
            self.local_mask_model.set_weights(self.local_mask_and_classifier)
        elif weight_or_mask == "weight":
            self.local_weight_model.set_weights(self.local_mask_and_classifier)
    ...
    def upload_gradients(self):
        ...
        with torch.no_grad():
            local_model_dict = self.local_weight_model.get_weights()
