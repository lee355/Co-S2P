...
def get_client_model(drop, training_mode, img_size, num_classes, embed_dim, transformer_depth, transformer_head, mlp_dim, device, 
    ...
    if training_mode == "mask":
        ...
        model = masked_dynamic_ViT_v2.ViT(training_mode="mask", drop=drop, image_size=img_size, patch_size=16, num_classes=num_classes, dim=embed_dim, 
                                depth=math.floor(model_rate*transformer_depth), full_depth=transformer_depth, 
                                heads=transformer_head, mlp_dim=mlp_dim, self_distillation=self_distillation, channels=3).to(device)
        #model.apply(init_param)
...
class FedModel:
    ...
    def __init__(self, drop, img_size, num_classes , embed_dim , transformer_depth , transformer_head, mlp_dim, lambda1, temperature, training_mode, 
        ...
        if lr is None:
            ...
            self.model, self.optimizer = get_client_model(drop=drop, training_mode=training_mode, img_size=img_size, num_classes=num_classes, segment_mask=segment_mask, 
                                                          embed_dim=embed_dim, transformer_depth=transformer_depth, transformer_head=transformer_head, 
    ...
    def train_weights(self, data_loader, local_sub_epoch, training_round):
        return self.new_weights_perform_local_epochs(data_loader, local_sub_epoch, training_round)

    def train_mask(self, data_loader, local_sub_epoch):
        return self.mask_perform_local_epochs(data_loader, local_sub_epoch)


    def weights_perform_local_epochs(self, data_loader, local_sub_epoch, training_round):
        loss = None
        for epoch in range(local_sub_epoch):
            ...
            for batch_idx, (train_x, train_y) in enumerate(data_loader):
                ...
                #train_x, targets_a, targets_b, lam = self.mixup_data(train_x, train_y, alpha=1.0)
            ...
            if epoch == local_sub_epoch-1:
                ...
                print("WEIGHT: train loss {}".format(train_loss))
        return loss

    def new_weights_perform_local_epochs(self, data_loader, local_sub_epoch, training_round):
        loss = None
        for epoch in range(local_sub_epoch):
            running_loss = 0
            total = 0
            ...
            for batch_idx, (train_x, train_y) in enumerate(data_loader):
                ...
                self.optimizer.zero_grad()
                y_pred = self.model(train_x)
                loss = criterion(y_pred[-1], train_y.long())
                for i in range(len(y_pred)-2,-1,-1):
                    gamma_active = training_round >= 10
                    loss += self.loss_fn_kd(y_pred[i], train_y.long(), y_pred[-1], gamma_active)
