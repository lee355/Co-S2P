...
class Client:
    ...
    def __init__(self, drop, img_size, num_classes, embed_dim, mlp_dim, transformer_depth, transformer_head, 
        ...
        self.local_model = FedModel(drop=drop, img_size=self.img_size, num_lcasses=num_classes, segment_mask=segment_mask,
                                    embed_dim=embed_dim, transformer_depth=transformer_depth, transformer_head=transformer_head, mlp_dim=mlp_dim, 
                                    lambda1=lambda1, temperature=temperature,
                                    training_mode="weight", device=self.device, self_distillation=self_distillation, 
                                    weight_decay=weight_decay, no_prox=no_prox, lr=lr, 
                                    model_rate=self.ratio, device=device)
