...
class Client:
    ...
    def __init__(self, drop, num_classes, embed_dim, mlp_dim, transformer_depth, transformer_head, lambda1, temperature, 
                 train_dataset, test_dataset, segment_mask, self_distillation, ratio, mask_round_ratio, img_size, device):
        ...
        self.local_model = FedModel(drop=drop, num_classes=num_classes, embed_dim=embed_dim, transformer_depth=transformer_depth, transformer_head=transformer_head, mlp_dim=mlp_dim, 
                                    lambda1=lambda1, temperature=temperature, ratio=self.ratio, img_size=self.img_size, 
                                    training_mode="weight", device=self.device, segment_mask=segment_mask, self_distillation=self_distillation, weight_decay=None, no_prox=None, lr=None, model_rate=1, device=None))
