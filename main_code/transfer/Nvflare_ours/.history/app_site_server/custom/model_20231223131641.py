...
class FedModel:
    """
        Federated model.
    """
    def __init__(self, drop, num_classes , embed_dim , transformer_depth , transformer_head, mlp_dim ,img_size, segment_mask, self_distillation, lambda1, temperature,
                ...
                for name, parameter in self.model.named_parameters():
                    if 'mask' in name:
                        ...
                        if flag:
                            ...
                            loss1 += torch.sum(g_m)
                            #loss1 += self.args.lambda1 * torch.nonzero(parameter).size(0)
                loss1 = self.lambda1 * torch.abs(loss1/mask_parameter_size - self.ratio)
                loss = loss + loss1
                running_loss += loss.item()
