...
class Server:
    ...
    def __init__(self, args, img_size, ratio_ls, device):

        self.args = args
        self.n_clients = self.args.num_users
        self.frac = self.args.frac
        self.clients_list = None
        self.device = device
        self.img_size = img_size
        self.ratio_ls = ratio_ls   #所有客户端的掩码比例
        self.round_global_weights = [None]     #记录每一轮开始时的全局模型参数
        self.training_mode = "both"  #mask, weight, both
