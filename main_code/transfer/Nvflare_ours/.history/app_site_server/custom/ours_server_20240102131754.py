...
class ScatterAndGather_Ours(Controller):
    def __init__(
        ...
        model_dict = torch.load("/home/xugw/pretrained_model/new_pretrained_model_with_patch2embed.pth")
    ...
    def control_flow(self, abort_signal: Signal, fl_ctx: FLContext) -> None:
        try:
            ...
            while self._current_round < self._start_round + self._num_rounds:
                ...
                self.log_info(fl_ctx,f"client_modelrate:{self.client_modelrate}")
                
                weights2client = {}
                for client_id, model_rate in  self.client_modelrate.items():
                    client_depth = int(model_rate * self.transformer_depth)
                    client_exist_num = int(model_rate * 4)
                    weights = {k: v.cpu().numpy() for k, v in self.server.global_model.model.state_dict().items() 
                               if "trasnformer_block" not in k and "exist_classifiers1" not in k}
                    for k, v in self.server.global_model.model.state_dict().items():
                        if "trasnformer_block" in k:
                            current_depth = eval(k.split('.')[1])
                            if (current_depth+1) <= client_depth:
                                weights[k] = v.cpu().numpy()
                        if "exist_classifiers1" in k:
                            current_exist_num = eval(k.split('.')[0][-1])
                            if current_exist_num <= client_exist_num:
                                weights[k] = v.cpu().numpy()
                                
                    weights2client[client_id] = weights
                    
                tasks = []
                for client in weights2client.keys():             
                    tmp_v = weights2client[client]
                    tmp_data = {"weight":tmp_v,"current_round":self._current_round}
                    tmp_dxo = DXO(data_kind="WEIGHTS", data=tmp_data)
                    tmp_shareable = tmp_dxo.to_shareable()
                    train_task = Task(
                ...
                print("----------------------------------------------------")
                for task_id,task in self._client_task_map.items():
                    print(task_id)
                    print(task.client)
                    print(task.task)
                print("----------------------------------------------------")
                ...
                '''weights = {k: v.cpu().numpy() for k, v in self.server.global_model.model.state_dict().items()}
                for name,parameters in self.server.global_model.model.named_parameters():
                ...
                self.server.aggregate_gradients(self.client_w, self.client_received_round, self._current_round,model_rate=self.client_modelrate)
                ...
                np.save('/home/xugw/result/loss.npy', self.loss_result)
                np.save('/home/xugw/result/top_1.npy', self.top_1_result)
                np.save('/home/xugw/result/f1_score.npy', self.f1_score_result)
                np.save('/home/xugw/result/resources_usage.npy', self.resources_usage_result)
                np.save('/home/xugw/result/top_5.npy', self.top_5_result)
    ...
    def _accept_train_result(self, client_name: str, result: Shareable, fl_ctx: FLContext) -> bool:
        ...
        self.client_received_round[client_name] = from_shareable(result).data['receive_round']
        self.client_time[client_name] = from_shareable(result).data['train_time']
        print(self.client_received_round[client_name])
        return True
