...
class Server:
    ...
    def compute_staleness(self, client_idxs, recevie_round, client_gradients_ls, n_round):
        ...
        '''
        client_gamma_ls = []
        assert len(client_idxs) == len(recevie_round)
        ...
        for i, (client_name, client_receive_round) in enumerate(recevie_round.items()):
            ...
            gradients_importance = sum(torch.sum(torch.abs(v)) 
                                       for v in client_gradients_ls[client_name].values()) / self.ratio_ls[client_idxs[i]]
    ...
    def aggregate_gradients(self, parallel_result, recevie_round, n_round):
        ...
        client_gradients_ls = []
        clinet_round_ls = []
        for client_id in self.current_aggregate_clients.keys():
            tmp = parallel_result[client_id]
            client_gradients_ls.append(tmp[0])
            clinet_round_ls.append(tmp[1])
        #print(client_gradients_ls[0])
        client_staleness_ls = self.compute_staleness(self.current_aggregate_clients, recevie_round, client_gradients_ls, n_round)
