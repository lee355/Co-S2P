        ...
        for k, v in self.global_model.model.state_dict().items():
            ...
            for m in client_modelrate.keys():
                ...
                if 'weight' in parameter_type or 'bias' in parameter_type:
                    if 'weight' in parameter_type:
                        if v.dim() > 1:
                            ...
                            if 'patch_to_embedding' in k:
                                ...
                                blocks = torch.chunk(v, chunks=4, dim=0)
                                sums = [block.abs().sum().item() for block in blocks]
                                roll = int(-1 * sums.index(max(sums)))
                                #roll = int(self.rounds) % output_size
                                ridx = torch.arange(output_size, device=v.device)
                                ridx = torch.roll(ridx, roll, -1)
                                #abs_sum = torch.sum(torch.abs(v), dim=(1,2,3))
                                #top_indices = torch.argsort(abs_sum, descending=True)[:local_output_size]
                                ...
                                ridx = torch.arange(output_size, device=v.device)
                                #ridx = torch.roll(ridx, roll, -1)
                                #abs_sum = torch.sum(torch.abs(v), dim=1)
                                #top_indices = torch.argsort(abs_sum, descending=True)[:local_output_size]'''
                                output_idx_i_m = ridx[:local_output_size]
                    ...
                    roll = int(self.rounds) % input_size
