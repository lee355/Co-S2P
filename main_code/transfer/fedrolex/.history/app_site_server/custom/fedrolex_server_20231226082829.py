...
class ScatterAndGather_fedrolex(Controller):
    def __init__(
        self,
        min_clients: int = 1,
        num_rounds: int = 5,
        ...
        self.test_loader,img_size = data_utils.get_serverdataloader('/home/xugw/fed_data/imagenet500/client_16non-iid_1.5/val')
    ...
    def control_flow(self, abort_signal: Signal, fl_ctx: FLContext) -> None:
        try:
            ...
            while self._current_round < self._start_round + self._num_rounds:
                ...
                weights2client ,self.param_idx= self.server.distribute(self.client_modelrate)
                ...
                self.server.aggregate_weights(self.client_w,self.param_idx,self._current_round)

                f1_score , val_loss , val_acc = self.evaluate(self.server.global_model.model,self.test_loader)
                ...
                self.log_info(fl_ctx,"[train round {}] loss: {:.3f}, acc: {:.3f} , f1_score :{:.3f} , resources_usage : {:.3f}".format(self._current_round,val_loss,val_acc,f1_score,resources_usage))
                ...
                self.log_info(fl_ctx,f"time : {round_time}")
    ...
    def _accept_train_result(self, client_name: str, result: Shareable, fl_ctx: FLContext) -> bool:
        ...
        self.client_w[client_name] = {k: torch.as_tensor(v,device=self.device) for k, v in from_shareable(result).data['weights'].items()}
