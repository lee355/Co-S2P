...
class Server:
    ...
    def split_model(self, client_modelrate):
        ...
        for k, v in self.global_model.model.state_dict().items():
            ...
            for client_idx, m in enumerate(client_modelrate.keys()):
                ...
                if 'weight' in parameter_type or 'bias' in parameter_type:
                    if 'weight' in parameter_type:
                        if v.dim() > 1:
                            ...
                            if 'patch_to_embedding' in k:
                                '''print('^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^')'''
                                input_size_2 = torch.arange(v.size(2) , device = v.device)
                                input_size_3 = torch.arange(v.size(3) , device = v.device)
                                input_idx_i_m = torch.arange(input_size, device=v.device)
                                local_output_size = int(np.ceil(output_size * scaler_rate))
                                '''print(local_output_size)'''
                                roll = int(client_idx/len(client_modelrate.keys())*output_size) % output_size
                                ridx = torch.arange(output_size, device=v.device)
                                ridx = torch.roll(ridx, roll, -1)
                                #random.seed(client_idx)
                                #top_indices = random.sample(list(range(output_size)),local_output_size)
                                ...
                                local_output_size = int(np.ceil(output_size // self.transformer_head
                                                                * scaler_rate))
                                #roll = int(self.rounds * local_output_size / 8 ) % output_size
                                roll = int(client_idx/len(client_modelrate.keys())*output_size) % output_size
                                ridx = torch.arange(output_size, device=v.device)
                                ridx = torch.roll(ridx, roll, -1)
                                ...
                                ridx = torch.arange(output_size, device=v.device)
                                ridx = torch.roll(ridx, roll, -1)
                                #random.seed(client_idx+len(client_modelrate.keys()))
                                #top_indices = random.sample(list(range(output_size)),local_output_size)
                                output_idx_i_m = ridx[:local_output_size]
                                idx_i[m] = output_idx_i_m
                            if 'patch_to_embedding' in k:
                                idx[m][k] = (output_idx_i_m,input_idx_i_m,input_size_2,input_size_3)
                            else:
                    ...
                    #roll = int(self.rounds * local_input_size / 8 ) % input_size
                    roll = int(client_idx/len(client_modelrate.keys())*input_size) % input_size
